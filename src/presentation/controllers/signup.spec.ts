...
describe('SignUp Controller', () => { // nome do componente que está testando
  ...
  test('Should return 400 if no name is provided', () => { // nome do teste que está sendo executado
    ...
    const httpResponse = sut.handle(httpRequest)
    expect(httpResponse.statusCode).toBe(400)
    expect(httpResponse.body).toEqual(new Error('Missing param: email')) // utilzar o toEqual para validar os valores de uma objeto, pois o toBe valida o pontore logo ambos os objetos tem que ser identicos
